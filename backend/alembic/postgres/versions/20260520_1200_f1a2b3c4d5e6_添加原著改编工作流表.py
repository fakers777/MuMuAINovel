...
def upgrade() -> None:
    ...
    if "adaptation_projects" not in table_names:
        ...
        if "user_id" not in existing_columns:
            ...
            op.execute(
                """
                UPDATE adaptation_projects ap
                SET user_id = p.user_id
                FROM projects p
                WHERE ap.project_id = p.id AND ap.user_id IS NULL
                """
            )
            op.execute("UPDATE adaptation_projects SET user_id = 'legacy-adaptation-user' WHERE user_id IS NULL")
            op.alter_column("adaptation_projects", "user_id", nullable=False)
        if "source_corpus_status" not in existing_columns:
            ...
            op.execute(
                ...
                SET source_corpus_status = CASE
                    WHEN source_filename IS NOT NULL AND btrim(source_filename) <> '' THEN 'completed'
                    ELSE 'pending'
                END
                WHERE source_corpus_status IS NULL
                """
            )
            op.alter_column("adaptation_projects", "source_corpus_status", nullable=False)
    ...
    existing_foreign_keys = {fk["name"] for fk in refreshed_inspector.get_foreign_keys("adaptation_projects")}
    if "fk_adaptation_projects_active_batch_id" not in existing_foreign_keys:
        op.create_foreign_key(
            "fk_adaptation_projects_active_batch_id",
            "adaptation_projects",
            "adaptation_planning_batches",
            ["active_batch_id"],
            ["id"],
    ...
    if "fk_adaptation_projects_last_confirmed_batch_id" not in existing_foreign_keys:
        op.create_foreign_key(
            "fk_adaptation_projects_last_confirmed_batch_id",
            "adaptation_projects",
            "adaptation_planning_batches",
...
def downgrade() -> None:
    op.drop_constraint("fk_adaptation_projects_last_confirmed_batch_id", "adaptation_projects", type_="foreignkey")
    op.drop_constraint("fk_adaptation_projects_active_batch_id", "adaptation_projects", type_="foreignkey")
